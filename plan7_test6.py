...
import time
def patch_id_mappings(job_id: str, output_dir: Path = Path("../savings/mid"), max_retries: int = 5, wait_time: int = 2):
    # Load the mappings file and wrap it in the required "extraction" key
    mappings_file_path = output_dir / f"{job_id}_mappings.json"
    
    retries = 0
    while not mappings_file_path.exists() and retries < max_retries:
        print(f"Mappings file not found for job {job_id}, retrying... ({retries + 1}/{max_retries})")
        time.sleep(wait_time)
        retries += 1

    if not mappings_file_path.exists():
        print(f"Mappings file not found for job {job_id} after {max_retries} retries.")
        return

    with open(mappings_file_path, "r") as f:
        mappings_content = json.load(f)

    # Check if mappings_content is empty
    if not mappings_content:
        # Send an empty extraction key
        ...
        if response.status_code == 200:
            print(f"POST request successful for job {job_id} with status {response.status_code}")
        else:
            print(f"POST request failed for job {job_id} with status {response.status_code}")
            print(f"Response content: {response.content.decode()}")
        return

    # Split the mappings content into chunks of 50
    chunk_size = 50
    ...
    for chunk in chunks:
        ...
        print(f"Sending chunk: {wrapped_content}")
...
async def send_patch(job_id: str, file: UploadFile = File(...)):
    # Read the uploaded file content
    file_content = await file.read()
    mappings_content = json.loads(file_content)
    ...
    if not mappings_content:
        ...
        return {"responses": [{"status_code": response.status_code, "content": response.content.decode()}]}
    ...
    for chunk in chunks:
        ...
        print(f"Received response: {response.status_code}, {response.content.decode()}")
